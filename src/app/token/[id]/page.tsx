...
function StatusBadge({ token }: { token: TokenRecord }) {
  ...
  if (token.status === "GRADUATED") return <span className="badge bg-white/10 text-white">✅ Выпустился</span>;
...
export default function TokenPage() {
  ...
  useEffect(() => {
    if (!id) return;
    void getToken(id).then(({ token }) => setToken(token)).catch((err: unknown) => setError(err instanceof Error ? err.message : "Токен не найден"));
  ...
  const progress = Math.round(token.state.progress * 100);
  const shareToTelegram = () => app?.openTelegramLink(`https://t.me/share/url?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(`🚀 ${token.name} ($${token.ticker}) на TONK.MEM!`)}`);
  ...
  return (
    <div className="space-y-4 pb-24 pt-4">
      <div className="space-y-4 px-4">
        ...
        {token.status === "GRADUATED" ? <div className="overflow-hidden rounded-[24px] border border-[#00c896]/20 bg-[#0b1325]"><div className="relative min-h-[180px]"><Image src="/brand/img_08.jpg" alt="Graduated token" fill className="object-cover opacity-38" /><div className="absolute inset-0 bg-[linear-gradient(135deg,rgba(6,12,24,0.92),rgba(6,12,24,0.52))]" /><div className="relative flex min-h-[180px] flex-col justify-end p-5"><div className="inline-flex w-fit items-center gap-2 rounded-full border border-[#00c896]/30 bg-[#00c896]/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#86efac]">🎓 Graduated on STON.fi</div><h2 className="mt-3 font-display text-2xl font-bold text-white">Этот токен уже выпустился</h2><p className="mt-2 max-w-md text-sm leading-6 text-[#c4d7ef]">Кривая закрыта, ликвидность собрана, дальше — жизнь на STON.fi без дешёвого MVP вайба.</p></div></div></div> : null}
        ...
        <div className="glass-card space-y-2 p-4">
          ...
          <div className="flex justify-between text-xs text-[#8ba3c1]"><span>💎 {token.state.reserveTon.toFixed(2)} TON собрано</span><span>цель: 8 888 TON</span></div>
        </div>
