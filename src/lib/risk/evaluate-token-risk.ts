...
export function evaluateTokenRisk(token: ExternalTokenRecord): Pick<ExternalTokenRecord, "riskLevel" | "warnings"> {
  ...
  const hasLiquidity = Boolean((token.liquidityGram && token.liquidityGram >= 500) || (token.liquidityUsd && token.liquidityUsd >= 500));
  const hasVolume = Boolean((token.volume24hGram && token.volume24hGram >= 100) || (token.volume24hUsd && token.volume24hUsd >= 100));

  if (!token.verified) warnings.push("Unverified token");
  if (!token.dexes.length) warnings.push("No DEX route");
  if (!hasLiquidity) warnings.push("Low liquidity");
  if (!hasVolume) warnings.push("Low volume");
