...
export function BuySellBox({ token }: { token: TokenRecord }) {
  ...
  const sellQuoteData = useMemo(() => {
    if (!(numericAmount > 0) || token.status !== "BONDING") return null;
    try {
      return quoteSell(token.state, numericAmount, token.creatorTax);
    } catch {
      return null;
    }
  }, [numericAmount, token]);
  ...
  const submitBuy = async () => {
    ...
    const tx = await sendTransaction(
      buildBuyDraft({
        ...
        minTokensOut: BigInt(Math.max(0, Math.floor(buyQuote.tokenAmount)))
      })
    );
  ...
  const submitSell = async () => {
    ...
    if (!userJettonWalletAddress || !isTonAddress(userJettonWalletAddress)) {
      throw new Error("Jetton wallet for this token is not available yet");
    }
    if (!isTonAddress(token.contractAddresses.bondingCurve)) throw new Error("Pool address is not indexed yet");
    ...
    const tx = await sendTransaction(
      buildSellDraft({
        ...
        poolAddress: toMainnetAddress(token.contractAddresses.bondingCurve),
        tokenAmount: BigInt(Math.max(1, Math.floor(numericAmount * 1_000_000_000))),
        minTonOutNano: BigInt(Math.max(1, Math.floor(sellQuoteData.tonAmountNet * 1_000_000_000)))
      })
    );
  ...
  if (!wallet) {
    return (
      <div className="glass-card rounded-[24px] p-6 text-center">
        <h3 className="text-xl font-semibold text-white">Connect wallet to trade</h3>
  ...
  return (
    <div className="glass-card rounded-[24px] p-4 space-y-4">
      ...
      <div>
        <label className="mb-1.5 block text-xs text-[#8ba3c1]">{mode === "buy" ? "Amount in GRAM" : `Amount in ${token.ticker}`}</label>
        <div className="relative">
          <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.0" className="w-full rounded-xl border border-[#1e3a5f] bg-[#1a2235] px-4 py-3 pr-20 font-mono text-lg text-white focus:border-[#2aabee] focus:outline-none" />
          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-[#8ba3c1]">{mode === "buy" ? "GRAM" : token.ticker}</span>
        </div>
      </div>

      <div className="grid grid-cols-4 gap-2">
        {(mode === "buy" ? ["0.1", "0.5", "1", "5"] : ["100", "1000", "10000", "50000"]).map((val) => (
          <button key={val} onClick={() => setAmount(val)} className="rounded-lg border border-[#1e3a5f] bg-[#1a2235] py-2 text-xs text-[#8ba3c1] hover:border-[#2aabee] hover:text-white">
            {val}
          </button>
        ))}
      </div>
      ...
      <div className="rounded-xl bg-[#1a2235] px-4 py-3 text-sm text-[#c6d4ea] space-y-2">
        <div className="flex justify-between"><span>Current price</span><span>{activeQuote ? `${activeQuote.newState.currentPriceTon.toFixed(8)} GRAM` : "—"}</span></div>
        {mode === "buy" ? (
          <>
            <div className="flex justify-between"><span>Estimated tokens</span><span>{buyQuote ? buyQuote.tokenAmount.toFixed(2) : "—"}</span></div>
            <div className="flex justify-between"><span>GRAM in USD</span><span>{gramPrice ? `$${gramPrice.toFixed(2)}` : "—"}</span></div>
            <div className="flex justify-between"><span>Estimated USD value</span><span>{gramPrice && numericAmount > 0 ? formatTonUsd(numericAmount, gramPrice) : "—"}</span></div>
          </>
        ) : (
          <>
            <div className="flex justify-between"><span>Gross TON out</span><span>{sellQuoteData ? `${sellQuoteData.tonAmountGross.toFixed(6)} GRAM` : "—"}</span></div>
            <div className="flex justify-between"><span>Net TON out</span><span>{sellQuoteData ? `${sellQuoteData.tonAmountNet.toFixed(6)} GRAM` : "—"}</span></div>
            <div className="flex justify-between"><span>Jetton wallet</span><span className="max-w-[140px] truncate">{userJettonWalletAddress ? `${userJettonWalletAddress.slice(0, 6)}...${userJettonWalletAddress.slice(-4)}` : "—"}</span></div>
          </>
        )}
      </div>
      ...
      <button onClick={submit} disabled={loading || !activeQuote || token.status !== "BONDING"} className="btn-primary flex w-full items-center justify-center">
        {loading ? "Waiting for wallet signature..." : mode === "buy" ? "Buy with wallet" : "Sell with wallet"}
      </button>
