...
export function BuySellBox({ token }: { token: TokenRecord }) {
  ...
  return (
    <div className="glass-card rounded-[24px] p-4 space-y-4">
      <div>
        <label className="mb-1.5 block text-xs text-[#8ba3c1]">Amount in GRAM</label>
        <div className="relative">
          <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.0" className="w-full rounded-xl border border-[#1e3a5f] bg-[#1a2235] px-4 py-3 pr-16 font-mono text-lg text-white focus:border-[#2aabee] focus:outline-none" />
          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-[#8ba3c1]">GRAM</span>
        </div>
      </div>
      ...
      <div className="rounded-xl bg-[#1a2235] px-4 py-3 text-sm text-[#c6d4ea] space-y-2">
        <div className="flex justify-between"><span>Current price</span><span>{quote ? `${quote.newState.currentPriceTon.toFixed(8)} GRAM` : "—"}</span></div>
