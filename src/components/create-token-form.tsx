    ...
    <div className="space-y-4 pb-8">
      <div className="mx-4 glass-card p-4">
        ...
        <div onClick={() => fileInputRef.current?.click()} className="relative mx-auto flex aspect-square max-w-[180px] cursor-pointer flex-col items-center justify-center overflow-hidden rounded-2xl border-2 border-dashed border-[#1e3a5f] transition-all hover:border-[#0088cc]/50">
          {preview ? <Image src={preview} alt="preview" fill className="object-cover" /> : <><Image src="/brand/img_05.png" alt="upload" width={64} height={64} className="mb-2 opacity-60" /><p className="text-center text-xs text-[#8ba3c1]">💎 Перетащи<br />или нажми</p></>}
        </div>
      ...
      {successUrl ? <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 px-4"><div className="glass-card w-full max-w-md space-y-4 p-6 text-center"><Image src="/brand/img_09.png" alt="success" width={120} height={120} className="mx-auto" /><h2 className="font-display text-2xl font-bold text-white">Токен запущен! 🎉</h2><p className="text-sm text-[#8ba3c1]">{name} <span className="text-[#0088cc]">${ticker}</span> теперь торгуется</p><div className="flex gap-3"><Link href={successUrl} className="btn-primary flex-1 text-center">Смотреть</Link><button onClick={shareToTelegram} className="flex-1 rounded-xl border border-[#0088cc]/30 py-3 text-sm text-[#0088cc]">✈️ Поделиться</button></div></div></div> : null}
    </div>
  );
}
