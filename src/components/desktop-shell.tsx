        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-6 py-4">
          ...
          <nav className="flex items-center gap-2 rounded-full border border-white/8 bg-white/5 p-1.5">
            {navItems.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link key={href} href={href} className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium transition ${active ? "bg-white text-black" : "text-[#c7d5e8] hover:bg-white/8 hover:text-white"}`}>
          ...
          <div className="flex items-center gap-3">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/8 bg-white/5 px-3 py-2 text-xs text-[#c7d5e8]"><LivePill /></div>
